# Быстрое покрытие половины проекта через превью: Prefire
...
У скриншот-тестов есть две проблемы:
- Нужно вручную удалять файлы при обновлении
- Выполнение таких тестов на порядок дольше других юнит-тестов: секунды или десятые секунды вместо тысячных долей секунды. 

Обе проблемы можно решить отложив генерацию скринштово до пулл реквеста. Получается такой сценарий:
- Вы верстаете вьюшку и смотрите в превью
- Создаете пулл реквест с новым кодом
- CI запускает полный прогон тестов. Если появились новые картинки или устарели старые, но от создает ПР *в ваш ПР* с добавлением/исправлением картинок, остается только подтвердить изменений.  
...
@Comment {
    Статья на медиуме, а он заблокирован в РФ
}
...
Больше никаких отмазок, что бизнес не дает времени на тесты, ведь оно само все делается!

@Comment {
    Тестирование вот прям вообще всех вьюшек позволяет найти баги от использования Perceptible. Так же можно и другие проблемы находить
}
