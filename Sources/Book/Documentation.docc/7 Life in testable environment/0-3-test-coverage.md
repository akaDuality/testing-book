...
- Строчки справа от кода показывают насколько опасно работать с кодом. Если красное — тестов нет, будь внимателен. Зеленое — найди какие тесты это проверяли. Запусти тесты в начале работы, чтобы убедиться что все зеленое и работаешь поверх готового кода. Если нет — отключи тест
}
