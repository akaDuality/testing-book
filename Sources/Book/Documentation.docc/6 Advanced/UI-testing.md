...
## Что тестируют UI-тесты

Мы уже много чего протестировали через юнит-тесты: сценарии могут быть простыми и сложными, но если писать код тестируемо, то мы можем повторить целые сценарии и проверить связи между ними. Зачем же тогда нужны UI-тесты?

В наших тестах было важное ограничение: мы строили переходы между экранами так, чтобы можно было тестировать данные, но что происходит в UI - неизвестно и все еще можно ошибиться. Мы эмулировали сценарии в <doc:main-page-testing>, но реальные экраны могут потерять связи и сценарий не повторится. Получается, что все предыдущие тесты были очень близко к задаче разработчика, проверили детально что происходит с данными, но реальный пользователь все еще может столкнуться с разными ошибками **на UI-уровне**: при навигации, реакции кнопок на нажатия и т.п.
...
@Comment {
    наверное, еще что-то надо тут дописать
}
...
@Comment {
    Как написать UI-test через MCP
    https://github.com/InditexTech/mcp-server-simulator-ios-idb
    
    
    Как подменять сетевые ответы
    https://habr.com/en/companies/naumen/articles/970822/
}
